#!/usr/bin/env python3
"""
LVGL Icon Font Builder - SVG → TTF + LVGL binary fonts with incremental builds
Part of Open Control CLI Tools
"""
...
def load_config(config_path: Path) -> dict[str, str]:
    """Load config from .conf file (bash KEY="value" syntax)."""
    config = {}
...
def clean_svg(inkscape: str, src: Path, dst: Path, temp_dir: Path, padding_percent: float) -> bool:
    ...
    if original_viewbox:
        ...
        t3 = temp_dir / "3.svg"
        if not inkscape_run(inkscape, t2, t3, fit=True):
            return False
...
def main():
    ...
    if not need_rebuild and ttf_path.exists() and header_path.exists() and lvgl_files_exist:
        ...
        # Generate TTF
        log("Generating TTF")
        glyphs = generate_font(fontforge_cmd, svg_cache, ttf_path, config)
        if not glyphs:
            error("FontForge failed")
        success(f"{ttf_path.name} ({len(glyphs)} glyphs)")

        # Generate Icon.hpp
        generate_header(glyphs, header_path, font_sizes, config)
